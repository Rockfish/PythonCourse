...
class MovieDatabase(object):
    ...
    def __init__(self):
        ...
        self.connection = sqlite3.connect("movies.sqlite")
    ...
    def add_movie_data(self, data):
        ...
        values = (data["Title"], data["Year"], data["Plot"], data["Poster"])
        result = self.connection.execute(cmds.insert_movie, values)
        ...
        for actor in data["Actors"].split(','):
            ...
            if result is None:
                ...
                result = self.connection.execute(cmds.insert_actor, values)
            ...
            result = self.connection.execute(cmds.insert_actor_in_movie, values)
    ...
    def get_movies(self):
        ...
        cursor = self.connection.execute(cmds.get_movies)
        result = cursor.fetchall()
        return result

    def get_actors(self):
        "Get the data for all actors"
        cursor = self.connection.execute(cmds.get_actors)
        result = cursor.fetchall()
        return result
    ...
    def add_upload(self, title, date, filepath):
        values = (title, date, filepath)
        self.connection.execute(cmds.insert_upload, values)
        self.commit()

    def add_user(self, userid, password):
        values = (userid, password)
        self.connection.execute(cmds.insert_user, values)
        self.commit()

    def get_user(self, userid):
        cursor = self.connection.execute(cmds.get_user, (userid, ) )
        result = cursor.fetchone()
        return result
